...
class InstagramUrlValidatorTest {
    ...
    fun testProfileUrlParsing() {
        ...
        assertTrue(result.isValid)
        assertEquals(InstagramUrlType.PROFILE_PIC, result.type)
        assertEquals("cristiano", result.username)
    }

    @Test
    fun testHighlightUrlParsing() {
        val url = "https://www.instagram.com/stories/highlights/12345678901234567/"
        val result = InstagramUrlValidator.parseUrl(url)

        assertTrue(result.isValid)
        assertEquals(InstagramUrlType.HIGHLIGHT, result.type)
        assertEquals("12345678901234567", result.shortcode)
    }
