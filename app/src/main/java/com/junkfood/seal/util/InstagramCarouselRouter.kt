...
object InstagramCarouselRouter {
    ...
    sealed interface RoutingResult {
        /** The content is a single media item. Use the normal Seal download path. */
    ...
    fun routeFromPlaylist(
        ...
        entries.forEachIndexed { index, entry ->
            val itemUrl = entry.url ?: run {
                Log.w(TAG, "Skipping entry $index — no URL")
                return@forEachIndexed
            }
            val task = Task(url = itemUrl, preferences = preferences)
            // Rich ViewState: title shows author + item position so notifications/task
            // list display "(@username) Item 1 of 5" instead of a raw CDN URL.
        ...
        if (tasks.isEmpty()) {
            return RoutingResult.ParseFailed("All carousel entries were skipped (no URLs).")
        }

        Log.i(TAG, "Carousel enqueued: ${tasks.size}/${entries.size} tasks for $shortcode")
    ...
    /** Extracts Instagram shortcode from /p/, /reel/, or /tv/ URLs. */
